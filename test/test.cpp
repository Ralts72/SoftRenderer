...
TEST(WindowTest, Clear) {
     ...
     EXPECT_EQ(pixelColor, (Color4{ 0.5, 0.5, 0.5, 1 }));  // 验证像素颜色是否正确
}
...
TEST(WindowTest, SetPixel) {
    ...
    const Color4 pixelColor = surface.getPixelColor(40, 160);  // 获取像素颜色
    EXPECT_EQ(pixelColor, (Color4{ 1, 1, 0, 1 }));  // 验证像素颜色是否正确
}
